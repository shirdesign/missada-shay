...
import { BREAD_CHOICES, EXTRAS, REMOVABLE_INGREDIENTS } from '../data/menu'
    ...
    >
      ...
      >
        ...
        <div className="relative h-52 flex-shrink-0">
          ...
          <button onClick={onClose} className="absolute top-4 left-4 bg-black/60 text-white rounded-full p-2 hover:bg-black/80 transition-colors">
            <X size={20} />
          </button>
          {item.chefsPick && (
        ...
        <div className="overflow-y-auto flex-1 p-6">
            ...
            <div className="flex gap-2 mb-4 flex-wrap">
              {item.tags.map(tag => (
                <span key={tag} className="text-xs px-3 py-1 rounded-full bg-dark-red/20 text-dark-red-light border border-dark-red/30">
                  {tag === 'vegetarian' ? '🌿 צמחוני' : tag === 'vegan' ? '🌱 טבעוני' : ''}
                </span>
              ))}
            </div>
          )}
          ...
          {item.customizable && (
            ...
            >
              <span className="font-medium">רוצה לשנות משהו? 😊</span>
          ...
          <AnimatePresence>
            {showCustom && item.customizable && (
              ...
              >
                {/* Bread */}
                <div className="mb-4">
                  <h4 className="text-white/60 text-xs uppercase tracking-widest mb-2">סוג לחם</h4>
                  <div className="flex gap-2 flex-wrap">
                    {BREAD_CHOICES.map(b => (
                      <button
                        key={b.id}
                        onClick={() => setBread(b.id)}
                        className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${bread === b.id ? 'bg-dark-red text-white' : 'bg-bg-card-2 text-white/70 hover:bg-white/10'}`}
                      >
                        {b.name}
                      </button>
                    ))}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {EXTRAS.map(e => (
                      <button
                        ...
                        className={`px-3 py-2 rounded-xl text-sm text-right transition-colors flex items-center justify-between ${extras.includes(e.id) ? 'bg-dark-red text-white' : 'bg-bg-card-2 text-white/70 hover:bg-white/10'}`}
                      >
                        <span className="text-xs text-white/50">{e.price === 0 ? 'חינם' : `+🪙${e.price}`}</span>
                        <span>{e.name}</span>
                      </button>
                    ))}
                  </div>
                ...
                {/* Remove */}
                <div className="mb-4">
                  <h4 className="text-white/60 text-xs uppercase tracking-widest mb-2">בלי...</h4>
                  <div className="flex gap-2 flex-wrap">
                    {REMOVABLE_INGREDIENTS.map(ing => (
                      <button
                        key={ing}
                        onClick={() => toggleRemove(ing)}
                        className={`px-3 py-2 rounded-xl text-sm font-medium transition-colors ${remove.includes(ing) ? 'bg-white/20 text-white line-through' : 'bg-bg-card-2 text-white/70 hover:bg-white/10'}`}
                      >
                        {ing}
                      </button>
                    ))}
        ...
        <div className="p-6 border-t border-white/10 flex items-center gap-4">
          ...
          >
            <span className="text-lg font-black">{total === 0 ? 'חינם! 🎉' : `🪙${total}`}</span>
            <span className="text-lg">הוסף להזמנה</span>
          </motion.button>
        </div>
      </motion.div>
    </motion.div>
